...
Este análisis tiene como objetivo estimar abundancias poblacionales y tendencias en el tiempo de aves playeras en nueve sitios monitoreados por el Migratory Shorebirds Project (MSP), utilizando además bases de datos provenientes de dos esquemas de monitoreos adicionales, Censo Neotropical de Aves Acuáticas (CNAA) y Censo Costero de Aves Playeras (CCAP). 
...
En este flujo dejaremos como ejemplo la preparación de la base de datos de MSP. Las bases de datos de los otros dos esquemas de monitoreo están preparadas y cargadas en el repositorio (CCAPver.rda y CNAAver.rda). Estas están en el formato que se requiere para trabajar con el paquete spAbundance.

```{r librerias y archivos}
# Cargar librerias y archivos necesarios para el taller
library(abind)
library(corrplot)
library(dplyr)
library(ggforce)
...
Para este ejemplo, trabajaremos con los datos registrados durante los censos realizados en la temporada verano y comenzaremos por identificar los sitios con sus respectivas fechas de censos y especies observadas para determinar las ausencias.

```{r preparación bd MSP}
df$Date <- as.Date(df$Date) #Le damos formato de fecha a la columna Date

# Seleccionar columnas relevantes y disminuir el tamaño de la base
df <- df %>%
  select(Sampling.Unit.Name, Start.Time, End.Time, Spp, Common.Name,
         Scientific.Name, Count, Date)

# Filtrar los registros de la temporada que trabajaremos 
...
# Identificar las filas con '---' en el código de la especie, son censos sin observaciones
...
Ahora que tenemos preparadas las bases de datos con las que trabajaremos, debemos unificarlas y dejarlas en el formato que se necesita para trabajar con el paquete spAbundance.
...
Ahora que tenemos nuestros datos preparados, vamos a construir un modelo estadístico para estimar las abundancias y tendencias en el tiempo de las aves playeras seleccionadas. Utilizaremos un Modelo Linear Generalizado Mixto (GLMM) que puede manejar datos de conteo con muchos ceros y sobredispersión.
...
# Configuración de Valores Iniciales. Los valores iniciales son el "punto de partida" para el algoritmo
ms.inits <- list(
...
# Configuración de parámetros de muestreo. Estos controlan cómo el algoritmo explora el espacio de parámetros (Valores más altos = pasos más grandes, valores más bajos = pasos más pequeños).
ms.tuning <- list(
...
# Modelo nulo 
# El modelo nulo incluye solo un intercepto (~ 1). Se asume que la abundancia es constante para todas las especies a través del tiempo y entre esquemas de monitoreo. Es nuestro "modelo base", el más simple.
out.0 <- msAbund(
  formula = ~ 1, 
  data = data_list,
  inits = ms.inits,
...
# Modelo completo
# El modelo completo incluye las covariables (~ MSP + CNAA + CCAP + year), lo que le permite capturar diferencias entre esquemas de monitoreo y tendencias temporales.
out.mod <- msAbund(
...
# Para evaluar si las covariables incluidas en el modelo completo realmente mejoran nuestras predicciones, comparamos los modelos usando WAIC (Widely Applicable Information Criterion). 
# WAIC es una medida que balanza el ajuste del modelo con su complejidad. Valores más BAJOS de WAIC indican mejores modelos.
...
Una vez ejecutado el modelo, es importante verificar que funciona correctamente. Los diagnósticos nos permiten evaluar si nuestras estimaciones son confiables.
Los diagnósticos nos ayudan a entender:
...
```{r diagnosticos}
# 1. Posterior Predictive Check (PPC). Este análisis compara los datos observados con lo que el modelo predice
verificacion_predictiva <- ppcAbund(out.mod, fit.stat = 'freeman-tukey', group = 0)
print(summary(verificacion_predictiva)) # Si el valor de Bayesian p-value está entre 0.3 y 0.7, el modelo ajusta bien

# 2. Análisis de residuales. Los residuales son las diferencias entre lo observado y lo predicho
...
```{r resultados}
#Creamos las carpetas en las que guardaremis los resultados
...
n_species <- dim(mu_samples)[2]  # Número de especies (6 en nuestro caso)
...
Además generaremos gráficos para las especies. Un panel de 9 gráficos por especie, uno por cada sitio monitoreado y un gráfico general de estimación de abundancia para cada especie, considerando todos los sitios. 

```{r gráficos por especie}
# Función para crear gráficos de tendencia temporal por sitio para una especie específica. Estos gráficos nos permiten visualizar cómo cambia la abundancia de una especie en un sitio particular.
grafico_especie <- function(especie_nombre, sitio_nombre, data = mu_df_completo) {
  ...
  stats_datos <- data_filtrada %>%
    ...
    summarise(
      ...
      ic_inf_50 = quantile(abundancia, 0.25), # Límite inferior intervalo 50%
      ic_sup_50 = quantile(abundancia, 0.75), # Límite superior intervalo 50%
      .groups = 'drop'
    )
  # Crear el gráfico
  p <- ggplot(stats_datos, aes(x = año)) +
    # Banda gris clara: zona de mayor incertidumbre (95% de los datos
    geom_ribbon(aes(ymin = ic_inf_95, ymax = ic_sup_95), alpha = 0.2, fill = "lightgray") +
    # Banda gris oscura: zona de menor incertidumbre (50% de los datos)
    geom_ribbon(aes(ymin = ic_inf_50, ymax = ic_sup_50), alpha = 0.4, fill = "darkgray") +
    # Línea azul: tendencia de la abundancia media a través del tiempo
    geom_line(aes(y = media), color = "darkblue", linewidth = 1.5) +
    # Línea roja punteada: tendencia de la mediana (menos influenciada por valores extremos)
    geom_line(aes(y = mediana), color = "darkred", linewidth = 1, linetype = "dotted") +
    theme_minimal() +
    theme(
      axis.text = element_text(size = 9, family = "Times"),
      axis.title = element_text(size = 10, family = "Times"),
      plot.title = element_text(size = 11, hjust = 0.5, family = "Times", face = "bold"),
...
# Para cada especie generaremos un panel con los 9 gráficos, uno por sitio.
panel <- function(especie_nombre, data = mu_df_completo, guardar = TRUE) {
  
  cat(paste("Procesando especie:", especie_nombre, "\n"))
  
  plots_sitios <- list()
  
  for(sitio in sitios_unicos) {
    p_sitio <- grafico_especie(especie_nombre, sitio, data)
    if(!is.null(p_sitio)) {
  ...
  titulo_panel <- paste("Abundancia por Sitio -", especie_nombre)
  subtitulo_panel <- "Línea azul: media | Línea roja punteada: mediana | Bandas grises: intervalos de confianza (claro=95%, oscuro=50%)"
  ...
  if(guardar) {
    ggsave(
      filename = paste0("resultados_abundancia/por_especie/", especie_nombre, "/abundancia_sitios_", especie_nombre, ".png"),
      plot = panel,
...
estpop <- function(especie_nombre, data = mu_df_completo, guardar = TRUE) {
  
  data_especie <- data[data$especie == especie_nombre, ]
  # Calcular la abundancia total por año sumando todos los sitios
  # Para cada muestra del modelo bayesiano y cada año, sumamos las abundancias  
  estimacion_poblacional <- data_especie %>%
    ...
    summarise(
      ...
      ic_inf_50 = quantile(abundancia_total, 0.25),
      ic_sup_50 = quantile(abundancia_total, 0.75),
      .groups = 'drop'
    )
  ...
  p_poblacional <- ggplot(estimacion_poblacional, aes(x = año)) +
    ...
    # Línea roja punteada: tendencia de la mediana (menos influenciada por valores extremos)
    geom_line(aes(y = mediana_poblacion), color = "darkred", linewidth = 1.5, linetype = "dotted") +
    theme_minimal() +
    theme(
    ...
    labs(
      ...
      title = paste("Estimación Poblacional -", especie_nombre),
      caption = "Línea azul: media | Línea roja punteada: mediana\nBandas grises: intervalos de confianza (claro=95%, oscuro=50%)"
    ) +
  ...
  if(guardar) {
    ggsave(
      filename = paste0("resultados_abundancia/por_especie/", especie_nombre, "/estimacion_poblacional_", especie_nombre, ".png"),
      plot = p_poblacional,
      width = 12, height = 8, dpi = 300, bg = "white"
    )
  }
  # Devolver tanto el gráfico como los datos calculados
  return(list(grafico = p_poblacional, datos = estimacion_poblacional))
}
...
paneles_especies <- list()
estimaciones_poblacionales <- list()

# Creamos todos los gráficos por especie
for(i in seq_along(especies_nombres)) {
  especie <- especies_nombres[i]
  
  # Panel por sitios (9 gráficos)
  panel_sitios <- panel(especie, data = mu_df_completo, guardar = TRUE)
  paneles_especies[[especie]] <- panel_sitios
  
  # Estimación poblacional general
  est_poblacional <- estpop(especie, data = mu_df_completo, guardar = TRUE)
  estimaciones_poblacionales[[especie]] <- est_poblacional
