...
df.u <- read.csv('UnidadesMSP.csv', sep = ";") #Base de datos con el detalle de los sitios MSP
...
año <- matrix(NA, nrow = length(all_SA), ncol = 1)
rownames(año) <- all_SA
colnames(año) <- "año"
...
for (i in 1:nrow(MSPverSitios)) {
  ...
  row_index <- match(SA, all_SA)
  
  año[row_index, ] <- year_value  
}


# Crear la matriz de sitio
sitio <- matrix(NA, nrow = length(all_SA), ncol = 1)
rownames(sitio) <- all_SA
colnames(sitio) <- "sitio"
# Llenar la matriz con los valores de sitio
for (i in 1:nrow(MSPverSitios)) {
  SA <- MSPverSitios$SitioAño[i]
  site_value <- MSPverSitios$Sitio[i]  
  
  row_index <- match(SA, all_SA)
  
  sitio[row_index, ] <- site_value  # ✓ Corregido
}

# Crear la matriz de superficie
Ha <- matrix(NA, nrow = length(all_SA), ncol = 1)
rownames(Ha) <- all_SA
colnames(Ha) <- "Ha"  # ✓ Corregido el nombre de columna
# Llenar la matriz con los valores de superficie
for (i in 1:nrow(MSPverSitios)) {
  SA <- MSPverSitios$SitioAño[i]
  Ha_value <- MSPverSitios$Ha[i]  
  
  row_index <- match(SA, all_SA)
  
  Ha[row_index, ] <- Ha_value  # ✓ Corregido
}
...
promedio_ajustado_matrix <- apply(valores_ajustados, c(2, 3), mean)

# Preparar datos para el análisis de residuales
datos_observados <- as.vector(data_list$y)
valores_predichos <- as.vector(promedio_ajustado_matrix)
...
cat("  Mínimo:", round(min(residuales_limpios), 3), "\n")
cat("  1er Cuartil:", round(quantile(residuales_limpios, 0.25), 3), "\n")
cat("  Mediana:", round(median(residuales_limpios), 3), "\n")
cat("  Media:", round(mean(residuales_limpios), 3), "\n")
cat("  3er Cuartil:", round(quantile(residuales_limpios, 0.75), 3), "\n")
cat("  Máximo:", round(max(residuales_limpios), 3), "\n")

# Gráficos del diagnóstico 
# Para los gráficos de residuales, necesitamos crear índice de valores finitos para aplicar consistentemente
indice_finitos <- is.finite(residuales) & !is.na(residuales)
pred_finales <- pred_validos[indice_finitos]
obs_finales <- obs_validos[indice_finitos]
residuales_finales <- residuales_limpios
...
with(plot(pred_finales, residuales_finales, 
...
with(qqnorm(residuales_finales, main = "Q-Q Plot de Residuales",
            pch = 16, col = rgb(0, 0, 1, 0.6)),
...
num_breaks <- min(30, max(5, length(residuales_finales)/20))

with(hist(residuales_finales,
     ...
     {
       x_norm <- seq(min(residuales_finales), max(residuales_finales), length = 100)
       y_norm <- dnorm(x_norm, mean(residuales_finales), sd(residuales_finales))
       
       # Escalar la densidad normal para que encaje con el histograma
       y_norm_scaled <- y_norm * length(residuales_finales) * diff(range(residuales_finales)) / num_breaks
       
       lines(x_norm, y_norm_scaled, col = "red", lwd = 2)
     })
