...
# Para observar mejor los resultados generaremos gráficos sin outlyers
...
grafico_especie <- function(especie_nombre, sitio_nombre, data = mu_df_completo) {
  ...
  stats_datos <- data_filtrada %>%
    ...
    summarise(
      media = mean(abundancia),
      mediana = median(abundancia),
      ic_inf_95 = quantile(abundancia, 0.025),
      ic_sup_95 = quantile(abundancia, 0.975),
      ic_inf_50 = quantile(abundancia, 0.25),
      ic_sup_50 = quantile(abundancia, 0.75),
      .groups = 'drop'
    )
...
estpop <- function(especie_nombre, data = mu_df_completo, guardar = TRUE) {
  ...
  p_poblacional <- ggplot(estimacion_poblacional, aes(x = año)) +
    ...
    labs(
      ...
      title = paste("Estimación Poblacional Nacional -", especie_nombre),
